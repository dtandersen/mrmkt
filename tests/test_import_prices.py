"""Unit tests for price import retry and partial-failure behavior."""

import unittest
from datetime import date

from mrmkt.command.prices_import import ImportPricesResult, ImportPricesUseCase
...
class TestImportPrices(unittest.TestCase):
    def setUp(self):
    ...
    def make_importer(self, source, dest, **kwargs):
    ...
    def test_retries_transient_failure_then_succeeds(self):
        ...
        importer = self.make_importer(source, dest)

        result = importer.execute(["AAPL"], self.start, self.end)
    ...
    def test_persistent_failure_isolates_batch_and_continues(self):
        ...
        dest = RecordingPriceDestination()
        importer = self.make_importer(source, dest)
        importer.batch_size = 1

        result = importer.execute(["AAPL", "MSFT"], self.start, self.end)
    ...
    def test_inverted_range_fails_before_any_request(self):
        ...
        with self.assertRaises(ValueError):
            importer.execute(["AAPL"], self.end, self.start)
