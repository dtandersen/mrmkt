...
from mrmkt.command.signals_current import CurrentSignalsRequest, render_csv

signals_app = typer.Typer(
    no_args_is_help=True, help="Inspect current strategy signals over stored prices"
)
...
def run_signals_current(
    ...
    as_of: str | None = typer.Option(
        None, "--as-of", help="Signal date (defaults to latest stored bar)"
    ),
    ...
    top: int | None = typer.Option(
        None, "--top", help="Keep only the first N symbol rows"
    ),
) -> None:
    """Show per-symbol strategy signals at a stored bar; prints deterministic CSV."""
    handle(
        ctx,
        lambda factory: factory.current_signals().execute(
