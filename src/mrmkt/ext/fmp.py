...
class FmpClient:
    ...
    def get_balance_sheet_statement(self, symbol, period="annual"):
        json = requests.get(
            f"{self.endpoint}/financials/balance-sheet-statement/{symbol}?period={period}"
        ).json()
        logging.debug(json)
        return json
    ...
    def get_income_statement(self, symbol, period="annual"):
        json = requests.get(
            f"{self.endpoint}/financials/income-statement/{symbol}?period={period}"
        ).json()
        logging.debug(json)
        return json
    ...
    def get_cash_flow(self, symbol: str, period="annual"):
        json = requests.get(
            f"{self.endpoint}/financials/cash-flow-statement/{symbol}?period={period}"
        ).json()
        logging.debug(json)
        return json
    ...
    def get_enterprise_value(self, symbol, period="annual"):
        json = requests.get(
            f"{self.endpoint}/enterprise-value/{symbol}?period={period}"
        ).json()
        logging.debug(json)
        return json

    def get_historical_price_full(self, symbol: str, start: date | None = None):
        ...
        json = requests.get(
            f"{self.endpoint}/historical-price-full/{symbol}?{y}"
        ).json()
        logging.debug(json)
        return json

    def get_stocks(self):
        json = requests.get(f"{self.endpoint}/company/stock/list").json()
        logging.debug(json)
        return json


class FMPReadOnlyFinancialRepository(
    ReadOnlyFinancialRepository, ReadOnlyPriceRepository, ReadOnlyTickerRepository
):
    ...
    def cnvt_income(self, symbol: str, income_stmt_json) -> IncomeStatement:
        if (
            "Weighted Average Shs Out (Dil)" in income_stmt_json
            and income_stmt_json["Weighted Average Shs Out (Dil)"] != ""
        ):
            waso2 = float(income_stmt_json["Weighted Average Shs Out (Dil)"])
        elif (
            "Weighted Average Shs Out" in income_stmt_json
            and income_stmt_json["Weighted Average Shs Out"] != ""
        ):
    ...
    def list_cash_flows(self, symbol: str) -> list[CashFlow]:
        ...
        return [
            FMPReadOnlyFinancialRepository.map_cash_flow(row, symbol)
            for row in json["financials"]
        ]
    ...
    def map_cash_flow(json, symbol: str) -> CashFlow:
        return CashFlow(
            ...
            dividend_payments=float(
                json["Dividend payments"] if json["Dividend payments"] != "" else 0
            ),
        )
    ...
    def list_enterprise_value(self, symbol: str) -> list[EnterpriseValue]:
        ...
        if "enterpriseValues" not in json:
            return []
        return [
            FMPReadOnlyFinancialRepository.map_enterprise_value(row, symbol)
            for row in json["enterpriseValues"]
        ]
    ...
    def closing_price(self, symbol, date) -> float | None:
    ...
    def get_price_on_or_after(self, symbol: str, date: date) -> StockPrice:
        raise NotImplementedError

    def get_income_statement(self, symbol: str, date: date) -> list[IncomeStatement]:
        raise NotImplementedError

    def get_balance_sheet(self, symbol, date: date) -> list[BalanceSheet]:
        raise NotImplementedError

    def get_cash_flow(self, symbol: str, date: date) -> CashFlow:
        raise NotImplementedError

    def get_enterprise_value(self, symbol: str, date: date) -> EnterpriseValue:
        raise NotImplementedError
